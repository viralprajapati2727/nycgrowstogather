    ...
    <div class="breadcrumb-line breadcrumb-line-light header-elements-md-inline">
        <div class="d-flex">
            <div class="breadcrumb">
                <a href="{{ route('job-title.index') }}" class="breadcrumb-item"><i class="icon-home2 mr-2"></i>Pending Jobs</a>
                <span class="breadcrumb-item active">Pending Job Listing</span>
            </div>
...
<script>
    ...
    var approve_reject_link = "{{ route('admin.job.approve-reject') }}";

    var filter = "{{ route('admin.pending-job-filter') }}";

$(document).ready( function () {
    pendingJobTable = $('#datatable').DataTable({
        serverSide: true,
        bFilter:false,
        ajax: {
            url: filter,
    ...
    $(document).on('click','.deleted', function() {
        var id= $(this).data('id');
        var pendingJobTable_row = $(this).closest("tr");
        swal({
                    ...
                    success: function(response) {
                        if(response.status == 200){
                            swal({
                                ...
                                pendingJobTable.row(pendingJobTable_row).remove().draw(false);
                            });
    ...
    $(document).on('click','.approve-reject', function() {
        ...
        var pendingJobTable_row = $(this).closest("tr");
        var dialog_title = (status == 1 ? "Are you sure you want to approve this job?" : "Are you sure you want to reject this job?");
