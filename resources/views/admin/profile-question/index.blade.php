    ...
    <div class="breadcrumb-line breadcrumb-line-light header-elements-md-inline">
        <div class="d-flex">
            <div class="breadcrumb">
                <a href="{{ route('profile-question.index') }}" class="breadcrumb-item"><i class="icon-home2 mr-2"></i>Questions</a>
...
<div class="card">
    <div class="card-header bg-white header-elements-inline">
        <h5>Question</h5>
        <a href=".add_modal" data-toggle="modal" data-backdrop="static" data-keyboard="false" class="btn btn-primary rounded-round add_question"><i class="icon-plus2"></i> Add New Question</a>
    </div>
    <div class="card-body">
        <div class="row">
            <div class="col-3">
    ...
    <div class="modal-dialog modal-md">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title question_title">Add Question</h5>
                <button type="button" class="close custom_close" data-dismiss="modal">×</button>
            </div>
            <form action="{{ route('profile-question.store') }}" method="POST" name="question_form" id="question_form" enctype="multipart/form-data">
                @csrf
                <div class="modal-body">
                    <div class="form-group row">
                        <label class="col-form-label pl-2 question_label">Add Question:<span class="text-danger">*</span></label>
                        <div class="input-group custom-start col-md-12">
                            <input type="text" value="" name="title" placeholder="Enter Question" class="form-control question "/>
                        </div>
                        <div class="input-group title-error-msg"></div>
                    </div>
                    <input type="hidden" name="id" class="question_id" id="d_type_id" value="">
                    <div class="form-group row">
                ...
                <div class="modal-footer">
                    ...
                    <input type="submit" class="btn bg-primary add-profile-question" value="Submit">
                </div>
            </form>
        </div>
    </div>
...
<script>
    ...
    var questionTable = "";
    var active_link = "{{ route('admin.change-profile-question-status') }}";

    var filter = "{{ route('profile-question-filter') }}";
    var oldImage = '';
    var flag = 1;


$(document).ready( function () {
    questionTable = $('#datatable').DataTable({
        serverSide: true,
        bFilter:false,
        ajax: {
            url: filter,
    ...
    $(document).on('click','.openQuestionPopoup',function() {
        var id = $(this).data('id');
        var title = $(this).data('title');
        $('.add_modal').modal({backdrop: 'static', keyboard: false});
        var path = "";
        $('.add_modal .question_id').val(id);
        var questionId = $('.question_id').val();
        if(questionId){
            $('.add_modal .question').val(title);
        }
    })
    ...
    $(document).on('click','.add_question',function(){
        $('.question_title').text('Add Question');
        $('.question_label').html('Add Question: <span class="text-danger">*</span>');
    })

    $(document).on('click','.edit_question',function(){
        var $this = $(this);
        $('.question_title').text('Edit Question');
        $('.question_label').html('Edit Question: <span class="text-danger">*</span>');
    ...
    $(document).on('click','.type-status',function () {
        ...
        var dialog_title = (active == 1 ? "Are you sure you want to active question?" : "Are you sure you want to deactive question?");
    ...
    $(document).on('click','.question_deleted', function() {
        ...
        var dialog_title = 'Are you sure want to delete this question ?';
        if(deleted != ''){
            dialog_title = 'This question is already in use, Are you sure want to delete this question?'
        }
        var questionTable_row = $(this).closest("tr");
        swal({
            title: dialog_title,
            type: 'warning',
            showCancelButton: true,
                    ...
                    success: function(response) {
                        if(response.status == 200){
                            swal({
                                ...
                                questionTable.row(questionTable_row).remove().draw(false);
                            });
        ...
        $("#question_form").validate({
            ...
            messages: {
                title: {
                    ...
                    // alpha: "The question may only contain letters and spaces.",
                    minlength: jQuery.validator.format("At least {0} characters required"),
                    maxlength: jQuery.validator.format("Maximum {0} characters allowed"),
                },
            },
            submitHandler: function (form) {
                $('.add-profile-question').attr("disabled", true);
                form.submit();
            }
        });
    });
